...
그러나 무엇보다도 눈에 띄는 것은 로건의 신체 너머로 뿜어져나오는 분위기였다.
같은 쥐인 제리는 더욱 강렬하게 느낄 수 있었다. 
부자가 키우는 애완쥐, 아니 어떤 애완동물일지라도 지금 로건이 뿜어내는 기백, 풍채를 가질 수는 없을 것이다.
...
톰과 제리는 갑작스런 로건의 말에 당혹감을 감추지 못했다.

```
시험이란건 간단해. 바로 너희들의 도덕심을 테스트해보는 거야.
...
```
방금 너가 이 세계의 조력자라고 했지? 넌 여기서 무엇에 조력하고 있는 거야?
```
...
톰 또한 자신의 친구라도 이곳에서 벗어나길 바랐다. 자신이 아니었더라면 제리는 이런 고생을 하지 않았을 것이다.
...
노을 너머 세계에 갇힐거란 걸 생각하면 막막해졌지만 제리를 구할 수 있다면 충분하다고 결론지은 것이다.
...
다리도 다친데다 착지의 충격으로 몸을 제대로 가눌 수 없는 상태였기 때문이다.
제리는 마음대로 움직이지 않는 자기 몸이 원망스러웠다.
서서히 닫혀지는 문을 바라보며 그가 할 수 있는 것은 톰의 이름을 부르는 것뿐이었다.
제리는 간절히 외쳤다.
...
제리는 절뚝거리며 생각했다. 
```
내가 좀만 더 조심했더라면 같이 돌아갈 수 있었을 텐데. 이대로 영영 톰을 못 보는 건 아니겠지? 
```
...
남겨진 톰은 제리가 넘어간 문쪽 방향을 멍때리며 바라보고 있었다. 
...
```
아냐, 이러고 있을 시간이 없어. 
내 친구 제리라면 틀림없이 잭을 데리고 노을 너머 세계로 다시 올거야! 우선 이곳에서 나의 친구 잭을 되돌릴 방법을 찾아야해. 
그리고 다시 현실 세계로 돌아갈 수 있는 방법을 찾는거야! 그럼 다같이 다시 원래대로 돌아갈 수 있어! 할 수 있다 톰!!
```
...
다리가 아파 걸을 수 없을 정도로 제리의 다리는 심하게 다쳐있었지만, 그것도 모른 채 제리는 대장 바퀴를 찾고있었다. 
...
```
후후... 그건 너희들이 살아서 돌아오면 말해주마. 이 여정의 끝에 그것이 무엇인지 알게 될게야.
...
```
준비가 됐나 보구나. 마법진이 노을 너머의 세계와 연결 됐단다. 
...
```
(기껏 멋지게 전투태세에 돌입했는데, 지금 이 몸을 앞에 두고 뭣들 하고 있는거지?)
```
...
```
어이, 지금 이 몸과 대치 중이라는 사실은 잊고 있었나? 방심은 금물이다 애송이들.
```
미야미야는 자신이 무시당한 것 같은 느낌이 들어 화가 나 있던 상태였다.
...
기억하기 싫어!!!
```
미야를 언급하니 미야미야가 고통스러워하는 것 같았다.
미야미야는 그들의 소리가 듣기 싫어 지팡이를 뺏어 그들을 공격하려고 하였다.
```
톰, 지팡이를 뺏기면 우린 죽을 거야!
지팡이를 빼앗기지 않도록 도망쳐!
```
톰과 제리가 서둘러 도망치려 할 때, 잭은 미동도 하지 않고 미야미야를 바라보았다.
```
당신... 설마.
```
```
...
```
톰은 물고 있던 지팡이를 떨어뜨렸고, 방금 정신을 차린 제리는 벙쪄있었다. 
미야미야가 말했다.
```
거기 너희 둘도... 미야의 친구인가?
```
...
미야미야는 제리를 가볍게 비웃은 후 시선을 잭에게 돌렸다.
```
잭! 되게 오랜만에 보는데, 반갑게 반겨주지 않다니 실망인데?
```
잭은 계속 몸이 굳어있었다.

사실, 잭이랑 미야가 팔찌를 끊고 각자의 길을 간 지 얼마 지나지 않아서 잭은 이곳을 탈출할 방법을 발견하였다.
...
하지만 겁을 먹은 잭은 미야를 구할 용기가 나지 않았고, 왔던 길로 재빨리 돌아가야겠다고 선택을 하였다.
그러다가 지금의 미야미야를 만나게 되었고 저주에 걸려 원래 세계로 되돌아온 것이었다.
